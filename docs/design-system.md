...
Палитра переведена на облик **«pro-tool»** (см. `docs/ui-reskin-plan.md`, Шаг 1.1):
тёмная тема по умолчанию, глубокий near-black холст, видимая hairline-граница,
когезивные акценты по типам в духе GitHub-dark.
...
## Плотность и форма (pro-tool) — чек-лист домиграции

Облик «pro-tool» = компактный ритм и сдержанная форма. Радиусы/тени/кегль живут
на встроенной шкале Tailwind (переопределять их в `:root` нельзя), поэтому
плотность задаётся **классами при переводе поверхности на токены**, а не токенами.
При домиграции компонента приводим его к этим правилам:

| Аспект | Было (консюмерский вид) | Стало (pro-tool) |
| --- | --- | --- |
| Скругления карточек/панелей | `rounded-2xl` | `rounded-lg` / `rounded-md` |
| Иконочные контейнеры | `rounded-2xl`, крупные | `rounded-lg`, компактнее |
| Плотность строк/списков | `py-3`, широкие `gap-*` | `py-1.5…py-2`, `gap` теснее |
| Разделение блоков | мягкие тени `shadow`/`shadow-lg` | hairline `border-border`, тень `shadow-sm`/нет |
| Кегль | 14px | опция 13px (через `settings.fontSize`) |
| URL / команды / переменные | интерфейсный шрифт | `font-code` (моно-акцент) |

Правило-ориентир: **граница вместо тени**. Иерархию поверхностей задаём парой
`bg-surface` + `border-border` на `bg-background`, а не тенью.
