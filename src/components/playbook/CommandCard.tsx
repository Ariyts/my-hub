...
import type { PlaybookItem, PlaybookLanguage, PlaybookVariable, ChecklistStatus } from '../../types';
...
export function CommandCard({
  item, containerId, mode, variables, checklistStatus, onChecklistCycle,
}: Props) {
  ...
  if (editing) {
    return (
      <div className="rounded-xl border border-cyan-400/40 bg-slate-900/60 backdrop-blur p-3 space-y-2 animate-in">
        <div className="flex items-center gap-2">
          <select
            className="text-[11px] px-2 py-1.5 rounded-md border border-slate-700 bg-slate-950 text-slate-200 outline-none focus:border-cyan-400"
          ...
          <input
            ref={editCmdRef}
            className="flex-1 font-mono text-xs px-3 py-1.5 rounded-md border border-slate-700 bg-slate-950 text-slate-100 outline-none focus:border-cyan-400"
        ...
        <input
          className="w-full text-xs px-3 py-1.5 rounded-md border border-slate-700 bg-slate-950 text-slate-100 outline-none focus:border-cyan-400"
        ...
        <input
          ...
          onChange={(e) =>
            setEditData({ ...editData, tags: e.target.value.split(',').map((t) => t.trim()).filter(Boolean) })
          }
        ...
        <div className="flex items-center gap-2 justify-end">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-md text-xs text-slate-400 hover:bg-slate-800 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-2.5 py-1 rounded-md text-xs font-medium bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 transition-colors disabled:opacity-40"
          >
            Save
          </button>
        </div>
        ...
        {mode === 'engagement' && (
          <button
            ...
            className={cn(
              ...
              isDone ? 'bg-emerald-500/10 border-emerald-500/30 hover:bg-emerald-500/20' :
              isSkipped ? 'bg-slate-800/40 border-slate-700 hover:bg-slate-700/40' :
              'bg-slate-900/40 border-slate-800 hover:bg-slate-800/40'
            )}
            title={`Click to cycle: ${checklistStatus} → ${
              checklistStatus === 'pending' ? 'done' : checklistStatus === 'done' ? 'skipped' : 'pending'
            }`}
          >
            {isDone ? (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 px-3 pt-2.5 pb-1.5">
            <span
              ...
              style={{ background: `${langColor}25`, color: langColor, border: `1px solid ${langColor}40` }}
            >
              {langLabel}
            </span>
            ...
            {mode === 'engagement' && variables.length > 0 && (
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 font-medium">
                RESOLVED
              </span>
            )}

            <button
              onClick={() => updatePlaybookItem(containerId, item.id, { isFavorite: !item.isFavorite })}
            ...
            >
              <Star
                ...
                className={cn(
                  'transition-colors',
                  item.isFavorite ? 'text-amber-400 fill-amber-400' : 'text-slate-600 hover:text-amber-400'
                )}
              />
            </button>
          </div>
          ...
          <button
            onClick={() => handleCopy()}
            className={cn(
              'w-full text-left px-3 py-2.5 relative group/code focus:outline-none',
              isDone && 'line-through decoration-slate-600 decoration-1'
            )}
            title={mode === 'engagement' ? 'Click to copy rendered command' : 'Click to copy template'}
          >
            <div
              className={cn(
                'font-mono text-[12.5px] leading-relaxed whitespace-pre-wrap break-all text-slate-100',
                'transition-opacity',
                copied && 'opacity-40'
              )}
            >
            ...
            {copied && (
              <div className="absolute inset-0 flex items-center justify-center bg-emerald-500/10 backdrop-blur-[1px] animate-in">
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/20 border border-emerald-400/40">
            ...
            {!copied && (
              <div className="absolute top-2 right-2 opacity-0 group-hover/code:opacity-100 transition-opacity pointer-events-none">
                <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-800/90 border border-slate-700">
          ...
          {description && (
            <div className="px-3 pb-2">
              <p className={cn('text-[11.5px] leading-snug text-slate-400', isDone && 'line-through decoration-slate-600')}>
                {description}
              </p>
            </div>
          )}

          <div className="flex items-center justify-end gap-1 px-2 py-1.5 border-t border-slate-800/70 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity">
            {mode === 'engagement' && variables.length > 0 && renderedText !== template && (
              <button
                onClick={() => handleCopy(template)}
                className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] text-slate-400 hover:bg-slate-800 hover:text-slate-200 transition-colors"
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] text-slate-400 hover:bg-red-500/15 hover:text-red-300 transition-colors"
            >
              <Trash2 size={11} />
              Delete
            </button>
