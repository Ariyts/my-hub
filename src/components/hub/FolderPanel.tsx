...
import type { Folder, BaseDataType } from '@/lib/types';
import {
  FolderOpen, Plus, Search, ChevronRight,
  ChevronDown, MoreVertical, FileText, Terminal, Link2, MessageSquare, Star, Folder as FolderIcon,
  Edit2, Trash2, GripVertical, File
} from 'lucide-react';
...
function FolderItem({ folder, depth, activeItemId, onSelectFolder, onSelectItem, onAddItemToFolder, onAddSubfolder }: FolderItemProps) {
  const { 
    ...
    categories,
    folders,
    notes, commands, links, prompts, files,
    activeFolderId, 
  ...
  const getItems = () => {
    switch (baseType) {
      ...
      case 'files': return files.filter(f => f.folderId === folder.id);
    }
  };
  ...
  const handleRenameItem = (itemId: string) => {
    if (newName.trim()) {
      switch (baseType) {
        ...
        case 'files': updateFileContainer(itemId, { title: newName.trim() }); break;
      }
    }
    setRenaming(null);
    setContextMenu(null);
  ...
  const handleDeleteItem = (itemId: string) => {
    switch (baseType) {
      ...
      case 'files': deleteFileContainer(itemId); break;
    }
    setContextMenu(null);
  };
    ...
    try {
      ...
      if (data && data.folderId !== targetFolderId) {
        switch (baseType) {
          ...
          case 'files': updateFileContainer(data.id, { folderId: targetFolderId }); break;
        }
      }
    } catch (err) {
  ...
  const handleAddItem = (folderId: string) => {
    switch (baseType) {
      ...
      case 'prompts':
        ...
        break;
      case 'files':
        addFileContainer({ folderId, title: 'New Files', subItems: [], tags: [], type: 'files', isExpanded: true });
        break;
    }
  };
