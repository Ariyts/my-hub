...
const BASE_TYPE_OPTIONS: { value: BaseDataType; label: string }[] = [
  ...
  { value: 'files', label: 'Files (Dropbox)' },
];
...
export function Sidebar() {
  ...
  const getItemCount = (categoryId: string) => {
    ...
    switch (category.baseType) {
      ...
      case 'files': 
        return files.filter(f => categoryFolderIds.includes(f.folderId)).length;
      default: 
        return 0;
    }
  };
  ...
  return (
    ...
    >
      {/* Workspace Header */}
      <div className="px-3 py-4 border-b" style={{ borderColor: isDarkTheme ? '#1e293b' : '#0f172a' }}>
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold" style={{ color: isDarkTheme ? '#e2e8f0' : '#fff' }}>
            📚 Knowledge Hub
          </h1>
          <button
            onClick={toggleTheme}
          ...
          >
            {isDarkTheme ? <Sun size={14} className="text-amber-400" /> : <Moon size={14} className="text-slate-300" />}
          </button>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        ...
        {workspaceCategories.map((category) => {
          ...
          return (
            <div key={category.id} className="relative group px-1">
              <button
